...
function stopMusic() {
	...
	if (ctx) teardownStem(ctx.currentTime);
}

function switchTrack(index: number) {
	if (!ctx || !musicBus) return;
	teardownStem(ctx.currentTime);
	trackIndex = index;
	sectionIndex = 0;
	stepIndex = 0;
...
function scheduler() {
	if (!ctx || !musicStarted || !prefs.musicOn) {
		schedulerId = null;
		return;
	}
	const track = TRACKS[trackIndex];
	...
	while (nextStepTime < horizon) {
		...
		scheduleStep(ctx, stepIndex, swung);
		stepIndex += 1;
		nextStepTime += stepDt;
...
function startMusic() {
	const audio = context();
	if (!audio || !musicBus || musicStarted || !prefs.musicOn) return;
	musicStarted = true;
	sectionIndex = 0;
	stepIndex = 0;
	section = makeSection(0);
